#include "PhaserApplication.h"

PhaserApplication::PhaserApplication()
{
    this->_image = ThumbnailImage::PHASER;
}

void PhaserApplication::run(sf::RenderWindow &mainWindow)
{
    checkIfRunnable();
    ...
    sf::Texture phaserImage;
    phaserImage.loadFromFile("PhaserProject/phaser.png");
    PhaserAnimatedSprite phaserSprite(phaserImage, 4, 12);
    phaserSprite.setTimeInterval(100);
    phaserSprite.setUpKey(sf::Keyboard::Up);
    phaserSprite.setDownKey(sf::Keyboard::Down);
    phaserSprite.setRightKey(sf::Keyboard::Right);
    phaserSprite.setLeftKey(sf::Keyboard::Left);
    ...
    while (isRunning)
    {
        ...
        phaserSprite.animate();
        phaserSprite.update(deltaTime.asSeconds());
        this->_textureToDrawOn->clear(sf::Color::Magenta);
        this->_textureToDrawOn->draw(phaserSprite);
